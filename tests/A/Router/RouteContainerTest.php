...
class RouteContainerTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->_route1 = new \A\Router\Route(array(
                                                    'name' => 'route1',
                                                    'route' => '*'
                                               ));

        $this->_route2 = new \A\Router\Route(array(
                                                    'name' => 'route2',
        ...
        $this->_route3 = new \A\Router\RouteStatic(
            new \A\Config\Config(array(
                'name' => 'route3',
                'route' => '/pages/about.html',
                'defaults' => array('module' => 'index', 'action' => 'about')
            ))
        );

        $this->_container = new \A\Router\Container();
    ...
    {
        $fakeRoute = new \A\Router\RouteStatic(array('name' => 'test', 'route' => 'test'));
        $this->assertInstanceOf('\A\Router\Container', $this->_container->addRoute($fakeRoute));
        $this->assertInstanceOf('\A\Router\RouteInterface', $this->_container->getRoute('test'));
        $this->assertInstanceOf('\A\Router\Container', $this->_container->removeRoute('test'));
